...
public class MazeGenerator : MonoBehaviour
{
    ...
    void PlaceTeleporters()
    {
        ...
        for(int i = 0; i < teleporterCount; i++)
        {
            do
            {
                x = HelperUtils.GetRandomNumber(1, MazeSize - 2);
                y = HelperUtils.GetRandomNumber(1, MazeSize - 2);
    ...
    void AStarConnect(Edge edge, int floor)
    {
        ...
        // open path
        OpenPath(pathMap, pathMap[dest.x, dest.y], floor);
    }

    // DIVIDE INTO 2 METHODS
    void OpenPath(AStarNode[,] pathMap, AStarNode parent, int floor)
    {
        AStarNode pathDest = parent;
        AStarNode currNode = pathMap[pathDest.position.x, pathDest.position.y];
    ...
    void CalculateAndPlaceRoom(Room room, Rotation rotationType, int floor)
    {
        ...
        if (rotationType == Rotation.Front)
        {
            do
            {
                x = HelperUtils.GetRandomNumber(1, MazeSize - room.length - 1); // 1 - 15
                y = HelperUtils.GetRandomNumber(1, MazeSize - room.width - 1); // 1 - 16
        ...
        else if (rotationType == Rotation.Back)
        {
            do
            {
                x = HelperUtils.GetRandomNumber(room.length, MazeSize - 2); // 4 - 18
                y = HelperUtils.GetRandomNumber(room.width, MazeSize - 2); // 3 - 18
        ...
        else if (rotationType == Rotation.Left)
        {
            do
            {
                x = HelperUtils.GetRandomNumber(room.width, MazeSize - 2); // 3 - 18
                y = HelperUtils.GetRandomNumber(1, MazeSize - room.length - 1); // 1 - 15
        ...
        else if (rotationType == Rotation.Right)
        {
            do
            {
                x = HelperUtils.GetRandomNumber(1, MazeSize - room.width - 1); // 1 - 16
                y = HelperUtils.GetRandomNumber(room.length, MazeSize - 2); // 4 - 18
    ...
    Rotation GetRandomRotation()
    {
        ...
        int randomizer = HelperUtils.GetRandomNumber(values.Length-1);

        return values[randomizer];
    }

    // MAKE GETRANDOMNUMBER STATIC IN ANOTHER CLASS (HELPERUTILS)
}
